...
- **L01 What's in My B.O.B.?**
  ...
  - `L01/L01-assessment.html` — 學習評量（字彙、綜合測驗、閱讀、填充、造句）
  - `L01/L01-adv-assessment.html` — 進階評量（字彙、語法、綜合測驗、對話、閱讀、填充、句型）
...
- **L06 Getting Paid to Stay at Hotels**
  ...
  - `L06/L06-assessment.html` — 學習評量
  - `L06/L06-adv-assessment.html` — 進階評量

## 複習卷

- **R12 Lessons 1–2 Review**
  - `R12/R12-assessment.html` — 複習評量（第 1～2 課）
  - `R12/R12-adv-assessment.html` — 進階複習評量（第 1～2 課）
- **R34 Lessons 3–4 Review**
  - `R34/R34-assessment.html` — 複習評量（第 3～4 課）
  - `R34/R34-adv-assessment.html` — 進階複習評量（第 3～4 課）
- **R56 Lessons 5–6 Review**
  - `R56/R56-assessment.html` — 複習評量（第 5～6 課）
  - `R56/R56-adv-assessment.html` — 進階複習評量（第 5～6 課）
- **R16 Lessons 1–6 Review**
  - `R16/R16-assessment.html` — 總複習評量（第 1～6 課）
  - `R16/R16-adv-assessment.html` — 進階總複習評量（第 1～6 課）

## 課程開放／關閉管理（管理頁）

網站提供 `admin.html` 管理頁（首頁頁尾「🔧 課程管理」進入），可分別控制各課「隨堂測驗」「學習評量」與「進階評量」的開放或關閉：

- **關閉時**：首頁該課的「📝 隨堂測驗／📘 學習評量／📗 進階評量」按鈕隱藏，直接輸入網址也會顯示「本單元尚未開放」；字卡與 S+V 挑戰維持開放。
- **開放／關閉設定**：存於根目錄 `config.json`（`quizzes`、`assessments` 與 `assessments_adv` 三欄位，`true`＝開放）。首頁與測驗／評量頁都會讀取此檔。
- **複習卷**（R12/R34/R56/R16）僅有學習評量與進階評量，於管理頁下方「複習卷」區塊個別開關。
